...
| 15 | Secrets-broker scope (#10) | **Vain pgcrypto-store + CRUD + `delivery=env`; `delivery=proxy` lykätty** | `delivery=proxy` (GitHub-tokenin header-injektio) vaatii TLS-MITM:n, joka on jo arkkitehtuurissa lykätty (§11.5 päätös 11, `squid.conf` "later refinement", §11.4 MVP-suoja = App-scope + branch protection). Korkea-arvoiset verkkocredentiaalit hoidetaan token brokerin kautta (§7.3), eivät secrets-brokerin. #10 = `secret_value`-taulu + pgcrypto, `POST /v1/secrets` (`CapSecretsManage`), lease-aikainen `delivery=env`→`runnerexec.Spec.Env`. `delivery=proxy` validoidaan skeemassa mutta runner hylkää (not-yet-supported) → oma seuranta-issue ("Egress-proxy TLS-MITM + GH-tokenin header-injektio"). |
| 16 | Admin-dashboard scope (#11) | **UI + merge-policy-endpoint + browser-OAuth; secrets-backend #10:stä, runner-write erikseen** | (a) Backend: `PUT /v1/projects/{id}/merge-policy` (`CapMergePolicyManage`) toteutetaan #11:ssä (`project_handlers.go` olemassa). Secrets-asetus kuluttaa #10:n `POST /v1/secrets`-endpointtia → **#11 riippuu #10:stä**. Runner-hallinnan write-operaatiot (drain/disable) lykätään omaan issueen (§7 ei speksaa operaatioita). (b) Selainauth: täysi browser-OAuth-device-flow (`/v1/auth/device/*`, #5) tähän issueen — dashboard saa session-tokenin ja lähettää `Authorization: Bearer`. SSE-tail: token query-paramina tai fetch-pohjainen SSE (EventSource ei aseta headeria). |
| 17 | Dashboard auth + roolihaaroitus | **`GET /v1/me` + selain-device-flow + capability-driven UI** | Dashboard renderöi GitHub-device-flow:n suoraan selaimessa (POST `/v1/auth/device/start` → näytä `user_code` + `verification_uri` → polll `/v1/auth/device/poll`); session-token sessionStorageen. UI-näkyvyys haaroittuu **capability-listan** (`/v1/me`:n `capabilities[]`) mukaan, ei rooli-stringin — `§7`-taulu on ainoa totuuden lähde. SSE-logitailille `fetch` + `ReadableStream` (EventSource ei tue Authorization-headeria). (#11) |
| 18 | `merge_policy`-skeema | **`{label, conflict_resolution}` — kaksi tunnettua kenttää** | §8:n "PR_WATCH_MERGE_LABEL + conflict-flag" kiteytyy kahteen jsonb-kenttään: `label` (string, oletus prwatch-tasolla `"auto-merge"`) ja `conflict_resolution` (bool, ohjaa `prwatch.Decide`:n `enableConflictResolution`-argumenttia). `PUT /v1/projects/{id}/merge-policy` (admin-only, `CapMergePolicyManage`) hylkää tuntemattomat kentät (`DisallowUnknownFields`) jotta kirjoitusvirheet eivät katoa jsonb-arvoihin. (#11) |
| 19 | "Viimeisen mailin" GitHub-auth (push/PR) | **Malli C: push + PR luotetulla runnerilla kontin jälkeen** | Kontti muokkaa worktreetä (orkestraattori pysähtyy S9:ään, `HandoffAfterAgent`; commit siirtyi hostille — tarkennus päätöksessä 20); runner-host ajaa `git push` + PR-luonnin brokerin (§7.3) mintaamalla lyhytikäisellä App-tokenilla (`internal/runnergit`), lease-varmistus ennen jokaista sivuvaikutusta (§10/R5). Invariantti 3 säilyy ilman TLS-MITM:ää — token kulkee vain runner-prosessin muistissa ja git/gh:n ympäristössä, ei koskaan konttiin, argv:hen eikä levylle. Tämä sulkee päätös 15:n seuranta-aukon: egress-proxy-MITM + header-injektio (§11.3-kaavion Malli A) toteutetaan vasta jos jaettu untrusted-tenant-runner toteutuu — sama ehto kuin päätöksissä 11 ja 12. Jäännös: §11.4:n proxy-puolen karkea write/read-tarkistus (MITM:n sivutuote) jää pois; hyväksytty, koska MVP-vahvistus on App-scope + branch protection (päätös 12) ja proxy-tarkistus oli luokiteltu "vain karkeaksi". (#34) |
| 20 | Host-puolinen commit (Malli C -tarkennus) | **Credentiaali + commit + push kaikki luotetulla runner-hostilla kontin jälkeen** | Agentti muokkaa tiedostoja kontissa muttei voi ajaa gitiä siellä: worktreen `.git` on osoitin host-polkuun, jota ei mountata konttiin. Aiempi muotoilu (päätös 19) oletti kontin committaavan — ilman host-committia haara pushattiin tyhjänä ja PR-luonti kaatui ("No commits between…"). Runner-host ajaa `GitOps.Commit`in (S9b, eksplisiittinen committer-identiteetti) ennen pushia; jos agentti ei tuottanut diffiä, ajo blokataan (`no_changes`) eikä tyhjää haaraa pushata. Invariantti 3 säilyy ennallaan. (PR #43, §11.3.) |
| 21 | Keskusliikenne ohi egress-proxyn | **`NO_PROXY=<flowd-host>` konttiin + flowd `flow-egress`-verkkoon** | Orchestratorin keskuskutsu (run-configin haku + telemetria, autentikoituna lease-skoopatulla runner-tokenilla) on luotettu sisäinen yhteys, ei egress-rajauksen piirissä — default-deny-proxy katkaisi sen ("access denied"). `runnerexec` asettaa kontille `NO_PROXY`/`no_proxy`-arvon keskus-hostille ja flowd liitetään `flow-egress`-verkkoon, jotta kontti tavoittaa sen suoraan. Vain agentin ulospäin suuntautuva github/npm/anthropic-liikenne kulkee proxyn kautta. Ei heikennä §11.3:a — kontissa ei tässäkään ole credentiaalia. (PR #43, §11.2.) |
| 22 | Egress-rajaus: squidin domain-ACL, ei IP-allow-list | **Firewall sallii 80/443 ulos; squidin dstdomain-ACL = kova rajaus** | ipset-IP-allow-list resolvoi sallitut domainit IP:iksi vain käynnistyksessä; CDN-hostien (github/codeload/packagist) IP-rotaatio kaatoi composer/npm-lataukset ajoittain kesken ajon. Squid avaa upstream-yhteydet vain allow-listattuihin domaineihin, joten IP-tason deny oli redundantti ja hauras. Firewall sallii TCP 80/443 ulos ja default-denyaa muun; squid pysyy ainoana, lokitettuna choke pointina (§11.6). Ei-HTTP-egress on edelleen estetty. (PR #43, §11.2.) |
...
docker run --rm \
  ...
  -v <worktree>:/work          # ainoa kirjoitettava host-mount
  -v <claude-cred>:/run/claude-credentials.json:ro  # Malli B, read-only (§11.5)
  -e HTTPS_PROXY=http://egress-proxy:3128
  -e NO_PROXY=<flowd-host>     # keskusliikenne ohi proxyn (§11.2, päätös 21)
  flow-orchestrator:<ver> orchestrate <run-id>
```
Invariantti: ei host-mountteja paitsi per-ajo worktree (rw) ja Claude-credentiaali (ro, §11.5/päätös 11), EI Docker-socketia (`/var/run/docker.sock`).
...
1. **Egress allow-list (default-deny):** kova domain-rajaus on **squidin dstdomain-ACL** (`squid.conf`; sallitut mm. `github.com`, `api.github.com`, `registry.npmjs.org`, `repo.packagist.org`, `api.anthropic.com`). Proxy-kontin firewall sallii TCP 80/443 ulos ja default-denyaa muun liikenteen (päätös 22). Alkuperäinen ipset-IP-allow-list resolvoi domainit IP:iksi vain käynnistyksessä, ja CDN-hostien (github/codeload/packagist) IP-rotaatio kaatoi composer/npm-lataukset ajoittain — siksi IP-tason deny korvattiin squidin domain-ACL:llä, joka avaa upstream-yhteydet vain allow-listattuihin hosteihin. Squid pysyy ainoana, lokitettuna choke pointina (§11.6). Pohja: **Anthropicin Claude Code -devcontainerin `init-firewall.sh`** (tuotteistettu referenssi).
2. **Credentials-injektio (§11.3).**
3. **Scoped git-write:** push vain `auto-run/*`-haaroihin, estää force-pushin mainiin + haaran poiston.

**Poikkeus — keskusliikenne ohi proxyn (NO_PROXY, päätös 21):** orchestratorin kutsut keskukseen (run-configin haku + telemetria, autentikoituna lease-skoopatulla runner-tokenilla) ovat luotettua sisäistä liikennettä, eivät egress-rajauksen piirissä. `runnerexec` asettaa kontille `NO_PROXY`/`no_proxy`-arvon keskus-hostille, ja flowd liitetään samaan `flow-egress`-verkkoon, jotta kontti tavoittaa sen suoraan. Ilman tätä default-deny-proxy katkaisisi keskuskutsut ("access denied"). Vain agentin ulospäin suuntautuva liikenne (github/npm/packagist/anthropic) kulkee proxyn kautta — §11.3 ei heikkene, koska kontissa ei ole credentiaalia.
...
**Toteutettu malli (päätös 19, tarkennus päätös 20, #34):** yllä oleva kaavio kuvaa kanonisen Malli A:n (proxy-injektio TLS-MITM:llä), joka on lykätty jaetun untrusted-tenant-runnerin ehdolla (päätökset 11/12/15). MVP toteuttaa saman invariantin **Malli C:llä**: agentti vain muokkaa tiedostoja kontissa — se ei voi ajaa gitiä siellä, koska worktreen `.git` on osoitin host-polkuun, jota ei mountata konttiin. Kontin päätyttyä luotettu runner-host ajaa **commitin (`GitOps.Commit`, S9b, eksplisiittinen committer-identiteetti), `git push`:n ja PR-luonnin** brokerista (§7.3) mintatulla App-tokenilla (`internal/runnergit`), lease-varmistuksella ennen jokaista sivuvaikutusta (§10/R5). Jos agentti ei tuottanut diffiä, ajo blokataan (`no_changes`) eikä tyhjää haaraa pushata. Credentiaali, commit ja push tapahtuvat siis kaikki luottamusrajan luotetulla puolella — token ei tässäkään mallissa koskaan ylitä luottamusrajaa konttiin.
...
- **Malli B (MVP, valittu — päätös 11):** Claude-auth elää kontissa tiedostona. Per-kone plan-billing rajaa vahingon koneen kiintiöön (ei tenant-dataan). **Todennettu R4-kokeella (§12).**
  ...
  **Konkreettinen polku (PR #43):** runner mounttaa credentiaalin read-onlyna konttiin polkuun `/run/claude-credentials.json` (`internal/runnerexec`; tiedosto, ei koskaan env-muuttuja, eikä keskus näe sitä). Orchestrator **stagettaa** sen käynnistyessään `$HOME/.claude/.credentials.json`:iin — ainoaan polkuun, josta claude-code credentiaalia etsii. Ilman stagingia agentti raportoi "Not logged in", jokainen claude-kutsu palauttaa tyhjän ja ajo tuottaa tyhjän tuloksen. Jos mounttia ei ole (dev-ajo ilman Claudea), staging on no-op eikä kaada ajoa.
