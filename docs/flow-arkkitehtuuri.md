...
**Toteutettu malli (päätös 19, #34):** yllä oleva kaavio kuvaa kanonisen Malli A:n (proxy-injektio TLS-MITM:llä), joka on lykätty jaetun untrusted-tenant-runnerin ehdolla (päätökset 11/12/15). MVP toteuttaa saman invariantin **Malli C:llä**: kontti committaa worktreehen muttei pushaa; kontin päätyttyä luotettu runner-host ajaa `git push` + PR-luonnin brokerista (§7.3) mintatulla App-tokenilla (`internal/runnergit`), lease-varmistuksella ennen jokaista sivuvaikutusta (§10/R5). Token ei tässäkään mallissa koskaan ylitä luottamusrajaa konttiin.
